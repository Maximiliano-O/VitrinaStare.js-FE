...
export default {
  ...
  methods: {
    ...
    return;
      ...
      try {
        const response = await axios.post(`${import.meta.env.VITE_APP_EXPRESS_URL}/release`, {
    ...
    try {
      const url = `${import.meta.env.VITE_APP_EXPRESS_URL}/usersV2/random/${this.userID}/${n}`;
  ...
  try {
    for (let user of this.fetchedUsers) {
      const response = await this.axios.post(`${import.meta.env.VITE_APP_EXPRESS_URL}/release/${releaseId}/status`, {
        releaseID: releaseId,
        reviewerID: user._id, 
        isReviewed: false,
        isSafe: false,
    ...
    async requestValidation(releaseId) {
      try {
        const url = `${import.meta.env.VITE_APP_EXPRESS_URL}/send-emails`;
        const data = {
